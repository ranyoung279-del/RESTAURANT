...
// Kiểm tra đăng nhập (TẠM THỜI TẮT ĐỂ TEST)
...
if (isset($_POST['add_promotion'])) {
    ...
    $check_sql = "SELECT id FROM promotions WHERE coupon_code = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "INSERT INTO promotions (title, description, coupon_code, discount_type, discount_value, start_at, end_at, active, image_url) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssdssds", $title, $description, $coupon_code, $discount_type, $discount_value, $start_at, $end_at, $active, $image_url);
...
if (isset($_POST['update_promotion'])) {
    ...
    // Kiểm tra mã coupon trùng (trừ chính nó)
    $check_sql = "SELECT id FROM promotions WHERE coupon_code = ? AND id != ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("si", $coupon_code, $id);
    $check_stmt->execute();
    $result = $check_stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "UPDATE promotions SET title=?, description=?, coupon_code=?, discount_type=?, discount_value=?, 
                start_at=?, end_at=?, active=?, image_url=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssdssdsi", $title, $description, $coupon_code, $discount_type, $discount_value, 
                         $start_at, $end_at, $active, $image_url, $id);
